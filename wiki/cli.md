...
| `omo company` | `--listen 127.0.0.1:8090`, `--max-agents 12`, `--usage-cache-ttl 10m`, `--mock`, `--unsafe`, `--basic-auth USER:PASSWORD`, `--no-origin-check`, `--detached` / `-d` | Open the local [company dashboard](company.md). `--unsafe` disables dashboard token authentication; Basic auth and disabled Origin checks are trusted-network/reverse-proxy options. |
| `omo company stop` | None | Stop this `OMO_HOME`'s company and wait for owned office/shell cleanup. |
| `omo company autostart register` | Same dashboard flags as `omo company` (without `-d`) | Save these exact settings and enable autostart at the current user's next login. |
| `omo company autostart unregister` | None | Remove this home's login entry and saved arguments without stopping the current company. |
