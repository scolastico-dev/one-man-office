...
| `omo supervisor` | `--listen 127.0.0.1:8090`, `--max-agents 12`, `--usage-cache-ttl 10m`, `--mock`, `--unsafe`, `--basic-auth USER:PASSWORD`, `--no-origin-check` | Open the local [browser supervisor](browser-supervisor.md). `--unsafe` disables dashboard token authentication; Basic auth and disabled Origin checks are trusted-network/reverse-proxy options. |
