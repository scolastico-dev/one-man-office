...
func TestResumeSpawnsFromOfficeDirectoryUsesUserIdentity(t *testing.T) {
	dir := t.TempDir()
	...
	cmd.SetOut(&out)
	cmd.SetErr(&out)
	cmd.SetArgs([]string{"office", "resume-spawns"})
	if err := cmd.Execute(); err != nil {
		t.Fatal(err)
	}
	if got := <-called; got != "user" {
		t.Fatalf("resume identity = %q", got)
	}
	if got := out.String(); got != "full office spawning resumed\n" {
