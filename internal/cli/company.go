...
import (
	...
	"github.com/scolastico-dev/one-man-office/internal/companyservice"
...
func validateCompany(options company.Options) error {
	...
	return company.ValidateOptions(options)
}
...
func companyArgs(cmd *cobra.Command) []string {
	var args []string
...
func addCompanyCommand(root *cobra.Command) {
	var options company.Options
	var detached bool
	var backgroundID string
	cmd := &cobra.Command{Use: "company", Short: "Run a local web dashboard for trusted offices and shell terminals", Args: cobra.NoArgs,
		Long: "Run a local web dashboard for trusted offices and shell terminals.\nThe access URL grants command execution with your permissions.\nUse --detached to run in the background and 'company stop' to stop it.\nOne company runs per OMO_HOME; autostart registration applies at user login.",
		RunE: func(cmd *cobra.Command, _ []string) error {
			if err := validateCompany(options); err != nil {
				return err
			}
			...
			if detached {
				...
				return companyservice.StartDetached(ctx, executable, companyArgs(cmd), cmd.OutOrStdout())
			}
			var out io.Writer = cmd.OutOrStdout()
			if backgroundID != "" {
				out = nil
			}
			return companyservice.Run(ctx, options, out, backgroundID)
		}}
	companyFlags(cmd, &options)
	...
	stop := &cobra.Command{Use: "stop", Short: "Stop the company and clean up its owned offices and shells", Args: cobra.NoArgs, RunE: func(cmd *cobra.Command, _ []string) error {
		ctx, cancel := context.WithTimeout(cmd.Context(), 30*time.Second)
		defer cancel()
		err := companyservice.Stop(ctx)
		if errors.Is(err, companyservice.ErrNotRunning) {
			fmt.Fprintln(cmd.OutOrStdout(), "Company is not running.")
			return nil
		}
		if err != nil {
			return err
		}
		fmt.Fprintln(cmd.OutOrStdout(), "Company stopped.")
		return nil
	}}
	autostart := &cobra.Command{Use: "autostart", Short: "Register or unregister the company for user login"}
	var startupOptions company.Options
	register := &cobra.Command{Use: "register", Short: "Save these company settings and enable user login autostart", Args: cobra.NoArgs, RunE: func(cmd *cobra.Command, _ []string) error {
		if err := validateCompany(startupOptions); err != nil {
			return err
		}
		executable, err := os.Executable()
		if err != nil {
		...
		location, err := companyservice.RegisterAutostart(cmd.Context(), executable, append([]string{"company"}, companyArgs(cmd)...))
		if err != nil {
			return err
		}
		fmt.Fprintf(cmd.OutOrStdout(), "Company autostart registered for next login.\nEntry: %s\n", location)
		return nil
	}}
	companyFlags(register, &startupOptions)
	unregister := &cobra.Command{Use: "unregister", Short: "Disable user login autostart without stopping the running company", Args: cobra.NoArgs, RunE: func(cmd *cobra.Command, _ []string) error {
		if err := companyservice.UnregisterAutostart(cmd.Context()); err != nil {
			return err
		}
		fmt.Fprintln(cmd.OutOrStdout(), "Company autostart unregistered.")
		return nil
	}}
	...
	autoRun := &cobra.Command{Use: "autostart-run SETTINGS", Hidden: true, Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
		registration, err := companyservice.LoadAutostart(args[0])
		if err != nil {
			return err
		}
	...
	root.AddCommand(cmd, &cobra.Command{Use: "company-shell", Hidden: true, Args: cobra.NoArgs, RunE: func(cmd *cobra.Command, _ []string) error { return company.RunShell(cmd.Context()) }})
}
