...
func TestAutostartRegisterReplaceUnregisterPreservesLiteralArguments(t *testing.T) {
	...
	args := []string{"supervisor", "--listen=127.0.0.1:0", "--basic-auth=user:p a'ss\"$HOME`id`%&\\word", "--mock=false"}
	...
	updated := []string{"supervisor", "--listen=127.0.0.1:8099"}
...
func TestDesktopLaunchPassesLiteralPathsThroughNativeParser(t *testing.T) {
	...
	for {
		...
		case <-ticker.C:
			...
			if !reflect.DeepEqual(args, []string{"supervisor", "autostart-run", config}) {
