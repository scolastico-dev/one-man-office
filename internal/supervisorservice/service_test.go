...
func isolatedHome(t *testing.T) string {
	...
	return filepath.Join(home, "supervisor")
}
...
func awaitState(t *testing.T, dir string) runtimeState {
	...
	for {
		select {
		case <-deadline.C:
			t.Fatal("supervisor did not publish readiness")
		case <-ticker.C:
...
func TestManagedSupervisorAuthenticatesStopAndCleansState(t *testing.T) {
	dir := isolatedHome(t)
	ctx, cancel := context.WithCancel(context.Background())
	done := make(chan error, 1)
	go func() {
		done <- Run(ctx, websupervisor.Options{Listen: "127.0.0.1:0", MaxAgents: 3, Unsafe: true}, nil, "test-launch")
	}()
	t.Cleanup(func() {
		cancel()
		select {
	...
	if runtime.GOOS != "windows" {
		for _, name := range []string{"runtime.json", "supervisor.log"} {
	...
	if err := Run(context.Background(), websupervisor.Options{MaxAgents: 1}, io.Discard, ""); err == nil {
		t.Fatal("second supervisor started")
	}
...
func TestFailedRunReleasesOwnership(t *testing.T) {
	dir := isolatedHome(t)
	err := Run(context.Background(), websupervisor.Options{Listen: "invalid", MaxAgents: 1}, nil, "failed")
	if err == nil {
		t.Fatal("invalid listener accepted")
	}
	log, readErr := os.ReadFile(filepath.Join(dir, "supervisor.log"))
