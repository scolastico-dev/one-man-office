...
func installAutostart(executable, config string) (string, error) {
	command := strings.Join([]string{windows.EscapeArg(executable), "supervisor", "autostart-run", windows.EscapeArg(config)}, " ")
	// Windows limits Run values to 260 characters. The settings stay in JSON,
	// keeping passwords and long supervisor arguments out of the registry.
