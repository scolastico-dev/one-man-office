...
func TestDetachedCLIReadinessStopAndOwnedShellCleanup(t *testing.T) {
	...
	output, err := run("company", "-d", "--listen=127.0.0.1:0", "--max-agents=4", "--mock")
	...
	if output, err := run("company", "--detached", "--listen=127.0.0.1:0"); err == nil || !strings.Contains(output, "already running") {
		t.Fatalf("duplicate launch: %s %v", output, err)
	}
	var instance company.InstanceInfo
	...
	if output, err := run("company", "stop"); err != nil || !strings.Contains(output, "Company stopped.") {
		t.Fatalf("stop: %s %v", output, err)
	}
	if err := syscall.Kill(pid, 0); err != syscall.ESRCH {
		t.Fatalf("owned shell %d survived stop: %v", pid, err)
	}
	if output, err := run("company", "stop"); err != nil || !strings.Contains(output, "not running") {
	...
	if output, err := run("company", "-d", "--listen="+occupied.Addr().String()); err == nil || !strings.Contains(output, "failed to start") {
		t.Fatalf("bind failure: %s %v", output, err)
	}
	log, err := os.ReadFile(filepath.Join(dir, "company.log"))
	...
	args := []string{"company", "--listen=127.0.0.1:0", "--max-agents=7", "--usage-cache-ttl=3m0s", "--mock=false", "--unsafe=false", "--no-origin-check=true", "--basic-auth=user:p a'ss\"$HOME`id`%&\\word"}
	...
	auto := exec.Command(binary, "company", "autostart-run", config)
	...
	if output, err := run("company", "stop"); err != nil {
...
func TestCancellationDuringStartupHookReleasesOwnership(t *testing.T) {
	...
	manifest := `{"name":"startup","hooks":[{"event":"company_startup","lua":"start.lua"}]}`
	...
	go func() {
		done <- Run(ctx, company.Options{Listen: "127.0.0.1:0", MaxAgents: 1}, io.Discard, "canceled")
	}()
