...
func TestLaunchAgentKeepsArgumentsAsXMLStrings(t *testing.T) {
	...
	want := []string{entryName(config), executable, "company", "autostart-run", config}
	...
	if strings.Contains(string(data), "KeepAlive") {
		t.Fatal("autostart would restart an explicitly stopped company")
	}
}
...
func TestDesktopEntryEscapesPathsAndDoesNotEmbedSettings(t *testing.T) {
	...
	want := `Exec="/opt/omo tools/omo" company autostart-run "/tmp/\\$HOME\\` + "`" + `whoami\\` + "`" + `%%x\\\\dir/autostart.json"`
