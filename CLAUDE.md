...
- Direct PTY input through `omo type` is server-authorized for only the user, CEO, firefighter, and trusted plugins running under the reserved system identity. A sanitized request event is persisted before queuing or writing, and a delivery event follows the actual PTY write; both record the target and input size/key count, never the input payload. Input targeting the agent under recent human editing in a writable TUI peek waits for the configured debounce or for that view to become non-writable, preserving FIFO text/key boundaries. The TUI pending-input marker covers both queued direct input and mail notices.
