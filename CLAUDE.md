...
- Successful usage checks replace one durable `model_usage_snapshots` row per profile; Claude rows include both weekly and five-hour session windows so live and read-only TUI agent overviews share the latest values.
