...
- Direct PTY input through `omo type` is server-authorized for only the user, CEO, firefighter, and trusted plugins running under the reserved system identity. A sanitized request event is persisted before queuing or writing, and a delivery event follows the actual PTY write; both record the target and input size/key count, never the input payload. Input targeting an agent in a writable TUI peek remains queued until the user leaves that view; it is not timer-debounced. FIFO text/key boundaries and the final readiness check under session input ownership prevent collisions. `notifications.input_debounce` applies only to mail notices. The TUI pending-input marker covers both queued direct input and mail notices.
