...
if enabled and role == "product_manager" then
	local note = marker .. "\nBefore `omo done`, write one authored Markdown description file in `storage`; gather facts from merged child `omo job` results and review notes because you have the whole picture. Use `## Summary` (2–4 sentences), `## What changed` (bullets by area/file group), `## Why` (the user-facing problem and decision), `## How it was verified` (exact commands/outcomes, test counts, and measured manual values), `## Risks and follow-ups`, and `## Jobs` (job IDs/titles). Use an absolute body path; relative paths are rejected. Run once with `body=<absolute-path>` using `omo plugin trigger pullrequest create -- [repo=<key>] body=<absolute-path> \"<title>\"`, then include every returned URL and its created/updated label in `omo done`." .. title_and_jobs
...
elseif enabled and developer_like and data.merge_target == "asis" and string.match(branch, "%S") then
	local note = marker .. "\nBefore `omo done`, write an authored Markdown description file in the worktree or a temporary path, never elsewhere inside `.omo`. Use `## Summary` (2–4 sentences), `## What changed` (bullets by area/file group), `## Why` (the user-facing problem and decision), `## How it was verified` (exact commands/outcomes, test counts, and measured manual values), `## Risks and follow-ups`, and `## Jobs` (job IDs/titles). Use an absolute body path; relative paths are rejected. Run once with `body=<absolute-path>` using `omo plugin trigger pullrequest create -- [repo=<key>] body=<absolute-path> \"<title>\"`, then include the returned URL and its created/updated label in `omo done`." .. title_and_jobs
	if string.find(text, marker, 1, true) == nil then
		data.text = text .. "\n\n" .. note
	end
end
