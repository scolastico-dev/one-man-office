...
`branches.naming` defaults to `ai`: omo starts a short-lived branch-naming agent with the job brief, using the smoke-alarm role's profile selection. It returns a complete Conventional Commits-style branch name such as `feat/add-search` or `fix/login-timeout`. AI names do not use `branches.prefix`, so branches are grouped by their top-level change type. If the naming agent times out, omo falls back to the generated `<branches.prefix><job-id>` name so dispatch can continue. Set `branches.naming: generated` to always use that generated name. The naming instructions are customizable in `.omo/messages/branch_naming_goal.txt`, like the other supervisor-generated prompts.
